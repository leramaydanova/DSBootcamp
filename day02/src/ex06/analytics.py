...
class Research:
    ...
    class Analytics(Calculations):
        ...
        def predict_random(self, number):
            ...
            logging.info("Predicting tge observations of heads and tails")
            return predict

        def predict_last(self):
            logging.info("Predicting the last data heads and tails")
        ...
        def save_file(self, data, file_name, ext):
            ...
            logging.info("Saving the file with a report")
        ...
        with open(self.path, 'r') as file:
            if has_header:
                ...
                if len(header) != 2 or not header[0].isalpha() or not header[1].isalpha():
                    logging.error("Program returned an error")
    ...
    def send_message(self, success = True):
        token = "7768525197:AAFYkA5oe3tJuJqvN9Ljcn2k2c7Z0NEuCHc"
        url = "https://api.telegram.org/bot"
        cnannel_id = "-1002771541120"
        url += token
        method = url + "/sendMessage"
        text = "The report has been successfully created" if success else \
